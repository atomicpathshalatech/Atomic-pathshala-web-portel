...
import { ArrowLeft, ClipboardList, CalendarDays, LayoutDashboard, MessageSquarePlus, BookOpenCheck, PanelLeftClose, Trash2, Library } from "lucide-react";
...
export function Sidebar({
  ...
  return (
    <aside className="flex h-full w-full flex-col bg-slate-50 dark:bg-slate-900">
      ...
      <div className="border-t border-slate-200 p-3 space-y-2 dark:border-slate-700">
        <Link
          href={user.isAdmin || user.isScheduleManager || user.isQuestionBankViewer ? "/team" : "/dashboard"}
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white dark:bg-slate-800 dark:border-slate-700 px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition"
        >
          <ArrowLeft className="h-3.5 w-3.5 text-atomic-orange" />
          <span>Exit Guru (Back to Portal)</span>
        </Link>
