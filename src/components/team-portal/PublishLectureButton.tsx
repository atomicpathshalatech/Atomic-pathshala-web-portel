...
export function PublishLectureButton({ lectureId }: { lectureId: string }) {
  ...
  async function publish() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/team/lectures/${lectureId}/publish`, { method: "POST" });
      const json = await res.json();
      if (!res.ok || !json.success) {
        setError(json.error ?? "Could not publish this lecture.");
        return;
      }
      router.refresh();
    } catch {
