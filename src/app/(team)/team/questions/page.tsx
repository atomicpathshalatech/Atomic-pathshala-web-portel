...
export default async function QuestionBankPage({
  ...
  const where = {
    ...(searchParams.search
      ? { body: { contains: searchParams.search, mode: "insensitive" as const } }
      : {}),
    ...(searchParams.status ? { status: searchParams.status as "PENDING" | "VERIFIED" | "FLAGGED" } : {}),
    ...(searchParams.difficulty
      ? { difficulty: searchParams.difficulty as "EASY" | "MEDIUM" | "HARD" | "ADVANCED" }
      : {}),
  };

  const [questions, total, statusCounts, subjectCount] = await Promise.all([
    ...
    <div className="space-y-stack-lg max-w-7xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-gutter">
        <div className="glass-card p-6 rounded-2xl">
          <p className="text-on-surface-variant font-label-md mb-1">Verified</p>
          <h3 className="text-[28px] font-bold text-tertiary">{countFor("VERIFIED")}</h3>
        </div>
        <div className="glass-card p-6 rounded-2xl">
          <p className="text-on-surface-variant font-label-md mb-1">Pending Review</p>
          <h3 className="text-[28px] font-bold text-primary">{countFor("PENDING")}</h3>
        </div>
        <div className="glass-card p-6 rounded-2xl">
          <p className="text-on-surface-variant font-label-md mb-1">Flagged</p>
          <h3 className="text-[28px] font-bold text-error">{countFor("FLAGGED")}</h3>
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
                ...
                <tr key={q.id} className="hover:bg-surface-container-lowest/50 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className="font-label-md text-primary">{q.subject?.title ?? "Unclassified"}</span>
                      {q.chapter && <span className="text-label-sm text-on-surface-variant">{q.chapter.title}</span>}
                    </div>
                  </td>
                  <td className="px-6 py-5 text-label-sm">{q.difficulty}</td>
                  <td className="px-6 py-5">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${STATUS_STYLES[q.status]}`}
                    >
                      {q.status}
                    </span>
                  </td>
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {canVerify && <QuestionStatusActions questionId={q.id} currentStatus={q.status} />}
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="px-6 py-4 bg-surface-container-low/50 flex items-center justify-between">
          <p className="text-label-sm text-on-surface-variant">
            Showing {questions.length === 0 ? 0 : (page - 1) * pageSize + 1}-
            {(page - 1) * pageSize + questions.length} of {total} questions
          </p>
