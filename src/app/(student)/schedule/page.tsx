              ...
              {Object.entries(grouped).map(([dayKey, entries]) => (
                <div key={dayKey} className="space-y-4">
                  ...
                  {entries.map((s) => {
                    ...
                    const isToday = isTodayInIST(s.startsAt);
                    const canEnterWaitingRoom = s.type === "LIVE_CLASS" && (isToday || isLiveNow || s.startsAt.getTime() - now.getTime() <= 2 * 60 * 60 * 1000);
                    return (
                      <div
                        key={s.id}
                            ...
                            {s.type === "LIVE_CLASS" ? (
                              canEnterWaitingRoom ? (
                                <Link
                                  ...
                                  className={`block text-center w-full md:w-auto px-6 py-2.5 text-white text-xs font-bold rounded-xl shadow-md transition-all ${
                                    isLiveNow
                                      ? "bg-rose-600 hover:bg-rose-500 shadow-rose-600/25 animate-pulse"
                                      : "bg-blue-600 hover:bg-blue-500 shadow-blue-600/25"
                                  }`}
                                >
                                  {isLiveNow ? "🔴 Join Live Class" : "🚪 Enter Waiting Room"}
                                </Link>
                              ) : (
                                <button
                                  disabled
                                ...
                                >
                                  Scheduled for {formatISTDate(s.startsAt)}
                                </button>
                              )
                            ) : s.type === "TEST" ? (
                              <Link
