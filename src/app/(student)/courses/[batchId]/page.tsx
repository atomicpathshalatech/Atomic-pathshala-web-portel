...
export default async function BatchCoursePage({
  ...
  // 2. Query Database Batch (by ID, Code or Name)
  ...
  try {
    ...
    if (session?.user?.id) {
      ...
      if (dbBatch) {
        ...
        const access = await resolveBatchAccess(session.user.id, dbBatch.id);
        isEnrolled =
      ...
      <div className="space-y-6 max-w-5xl mx-auto py-4 sm:py-6 px-4">
        ...
        <div>
          <p className="flex items-center gap-2 text-xs text-slate-500 mb-2">
            ...
            <span className="text-[#031635] font-bold">{dbBatch.name}</span>
          </p>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {course.subjects.map((s: any) => (
              <Link
                key={s.id}
                href={`/courses/${dbBatch.id}/subjects/${s.id}`}
              ...
              >
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500">
                    {s.chapters?.length || 0} Published Chapters
                  </p>
                </div>
  ...
  const courseData: any = {
    ...
    discountPercentage:
      dbBatch?.originalPrice && dbBatch?.price && dbBatch.originalPrice > dbBatch.price
        ...
        : undefined,
    isNewBatch: true,
    thumbnailUrl: dbBatch?.thumbnailUrl ?? null,
    isEnrolled,
